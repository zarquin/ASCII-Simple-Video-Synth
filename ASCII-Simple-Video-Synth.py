...
redGen = Generator(0.5,0.01,255.0)
bluGen = Generator(0.5,0.002,255.0)
...
shape_drawer = ShapePoints(sides=4, xincrement=0.7, yincrement=0.123, size=0.7)
...
def ds(screen):
    ...
    global draw_method, draw_foreground, frame_count
    single_draw_delay = False
    framerate=1.0
    ...
    while True:
        ...
        if(time_delta < frame_time):
            ...
            time.sleep(diff)
        #need to take the frame-rate calculation after any sleeps to set the right frame rate.
...
def main():
    ...
    parser.add_argument("--framecount", type=int, default=-1,
        help="run for a fixed number of frames, then quit.  used for profiling and testing" )
